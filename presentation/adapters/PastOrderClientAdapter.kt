...
class PastOrderClientAdapter(private val orders: List<OrderEntity>,
                             private val getProductByIdUseCase: GetProductByIdUseCase):
    RecyclerView.Adapter<PastOrderClientAdapter.OrderClientViewHolder>() {

    class OrderClientViewHolder(val binding: ItemOrderClientBinding) : ViewHolder(binding.root){
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OrderClientViewHolder {
        val view = OrderClientViewHolder(ItemOrderClientBinding.inflate(LayoutInflater.from(parent.context),parent,false))
        return view
    }
    ...
    override fun onBindViewHolder(holder: OrderClientViewHolder, position: Int) {
        ...
        holder.binding.itemsRecyclerView.adapter = ItemsInOrderClientAdapter(order.orderItems,getProductByIdUseCase)
